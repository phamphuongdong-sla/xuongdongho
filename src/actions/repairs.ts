...
export async function submitRepairVoucher(data: {
  ...
  const ktv = await prisma.user.findFirst({ where: { role: 'ktv' } });
  ...
  if (!targetMeter) throw new Error('Đồng hồ không tồn tại.');
  if (
    targetMeter.code === 'ĐH150-CTOR' ||
    targetMeter.code.includes('150') ||
    targetMeter.name.toLowerCase().includes('150')
  ) {
    throw new Error('Xưởng chỉ sửa chữa các loại đồng hồ từ DN50 trở xuống. Không thể tạo phiếu sửa chữa cho đồng hồ DN150.');
  }
  ...
  if (data.inputQuantity < data.completedQuantity + data.scrappedQuantity) {
    throw new Error('Số lượng hoàn thành + hỏng loại bỏ không được vượt quá số lượng nhận sửa.');
  }

  // 2. Validate spare parts inventory constraints (AC 5.2)
  for (const p of data.partsUsed) {
    ...
    if (p.quantity > currentStock) {
      ...
      throw new Error(
        `Không đủ tồn kho cho linh kiện ${partInfo?.name ?? p.sparePartId} (Tồn hiện tại: ${currentStock}, Yêu cầu xuất: ${p.quantity}).`
      );
    }
  }
  ...
  await prisma.$transaction(async (tx) => {
    ...
    const voucher = await tx.repairVoucher.create({
      data: {
        ...
        technicianId: ktv?.id ?? admin?.id ?? 1,
        status: 'completed',
...
export async function updateRepairVoucher(data: {
  ...
  if (data.inputQuantity <= 0) throw new Error('Số lượng nhận sửa phải lớn hơn 0.');
  if (data.inputQuantity < data.completedQuantity + data.scrappedQuantity) {
    throw new Error('Số lượng hoàn thành + hỏng loại bỏ không được vượt quá số lượng nhận sửa.');
  }
  ...
  if (voucher.code.includes('BS') || voucher.notes?.includes('[Nguồn:')) {
    throw new Error('Phiếu xuất bổ sung phải được hoàn tác rồi lập lại để bảo toàn dấu vết kế hoạch.');
  }
  ...
  for (const part of normalizedParts) {
    ...
    if (part.quantity > availableAfterRevert) {
      ...
      throw new Error(`Không đủ tồn kho cho linh kiện ${partInfo?.name ?? part.sparePartId} (khả dụng sau hoàn phiếu: ${availableAfterRevert}).`);
    }
  }
...
export async function submitSupplementRepairVoucher(data: {
  ...
  if (data.quantity <= 0) {
    throw new Error('Số lượng bổ sung phải lớn hơn 0.');
  }
  ...
  if (data.quantity > availableNew) {
    ...
    throw new Error(
      `Không đủ tồn kho ĐH Mới tại Kho VP cho ${sourceMeter?.name || data.sourceMeterId} (Tồn mới hiện tại: ${availableNew}, Yêu cầu xuất: ${data.quantity}).`
    );
  }
...
export async function updateRepairVoucherNotes(id: number, notes: string) {
